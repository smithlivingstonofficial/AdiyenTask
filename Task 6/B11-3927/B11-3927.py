...
def convert_txt_to_json(input_path, output_path):
    ...
    # Find verse numbers like: || 1 ||, ॥ १॥, । । १, or a bare number at line end.
    verse_num_pattern = re.compile(r'(?:[\|॥।]+\s*([०-९0-9]+)(?:\s*[\|॥।]+)?|([०-९0-9]+)\s*$)')
    ...
    for block in blocks:
        ...
        if matches:
            ...
            raw_num = next(num for num in reversed(matches[-1]) if num)
            ...
            preview = block[:30].encode('unicode_escape').decode('ascii')
            print(f"Warning: Could not extract verse number from block starting with: {preview}...")
...
if __name__ == "__main__":
    ...
    input_file = os.path.join(script_dir, "processed", "3927.txt")
    output_file = os.path.join(script_dir, "json", "3927.json")
